...
from app.initializers import settings, DBTask
...
@celery.task(base=DBTask, name = 'app.core.decode.decode_collection')
def decode_collection(directory, collection, movie_name):
	for t in db.metadata.tables.items():
		print(t)
	
	decoding_set = db.session.query(DecodingSet).filter_by(name='terms_20k').first()
